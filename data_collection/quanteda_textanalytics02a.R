...
# ============================================================================
# SECTION 12: WORDFISH – ESTIMATING IDEOLOGICAL POSITIONS
# ============================================================================

cat("\n=== Estimating document positions with Wordfish ===\n")

# 1. Choose a subset of speeches (e.g., post-1949)
corp_wf <- corpus_subset(data_corpus_inaugural, Year > 1949)

# 2. Create a dfm (with some trimming/cleaning)
dfm_wf <- corp_wf |>
  tokens(remove_punct = TRUE) |>
  tokens_remove(stopwords("english")) |>
  dfm() |>
...
# 3. Fit Wordfish model
wf_model <- textmodel_wordfish(dfm_wf)

# 4. Inspect estimated positions (one per speech)
head(wf_model$theta)          # document positions
docvars(corp_wf, "President") # match to presidents

# 5. Plot document positions on the latent scale
textplot_scale1d(wf_model, margin = "documents") +
  ggplot2::labs(
    title = "Estimated Positions from Wordfish",
    x = "Latent Dimension",
    y = "Presidential Inaugural Addresses"
  )
